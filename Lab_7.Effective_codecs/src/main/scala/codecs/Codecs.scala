...
trait DecoderInstances:
  ...
  Decoder.fromPartialFunction { case Json.Bool(n) => n.booleanValue() }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      val decodeItem = decoder.decode(item)
      if (decodeItem.isDefined)
        Some(decodeItem.get +: previouslyDecodedItems)
      else None
...
case class Student(name: String, surname: String, id: Int)

object Student extends StudentCodecs

trait StudentCodecs:
  /** The encoder for `Student` */
  given studentEncoder: Encoder[Student] =
    ObjectEncoder
      .field[String]("name").zip(ObjectEncoder.field[String]("surname"))
      .zip(ObjectEncoder.field[Int]("id"))
      .transform[Student](student => ((student.name, student.surname), student.id))

  /** The decoder for `Student` */
  given studentDecoder: Decoder[Student] =
    Decoder.field[String]("name").zip(Decoder.field[String]("surname"))
      .zip(Decoder.field[Int]("id"))
      .transform[Student](student => Student(student._1._1, student._1._2, student._2))
...
@main def run(): Unit =
  ...
  val maybeJsonObj3 = parseJson(""" { "name": Serhii, "surname": Butko, "id": 23 } """)
  ...
  println(maybeJsonObj3.flatMap(_.decodeAs[Student]))
  println(renderJson(new Student("Serhii", "Butko", 23)))