...
class PostSetSuite extends munit.FunSuite {
  ...
  test("contains: with data") {
    val setInSet5 = new Post("a", "a body", 20)
    assert(set5.contains(setInSet5))
  }

  test("contains: without data") {
    val setNotInSet5 = new Post("e", "e body", 10)
    assert(!set5.contains(setNotInSet5))
  }
