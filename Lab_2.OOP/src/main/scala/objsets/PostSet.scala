...
class Empty extends PostSet {
  ...
  override def mostLiked: Post = throw new NoSuchElementException
...
class NonEmpty(elem: Post, left: PostSet, right: PostSet) extends PostSet {
  ...
  def filterAcc(p: Post => Boolean, acc: PostSet): PostSet = {
    val current = if (p(elem)) acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, current))
  }

  override def union(that: PostSet): PostSet = {
...
object GoogleVsApple {
  ...
  lazy val googlePosts: PostSet = PostReader.allposts.filter(p => google.exists(p.text.contains))
  lazy val applePosts: PostSet = PostReader.allposts.filter(p => apple.exists(p.text.contains))
