...
trait DecoderInstances {
  ...
  implicit val boolDecoder: Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(n) => n.booleanValue() }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) => {
        ...
        if (items.forall(i => i.isDefined))
          Some(items.map(i => i.get))
        else None
      }
    }
...
trait StudentCodecs {
  ...
  implicit val studentEncoder: Encoder[Student] =
    ObjectEncoder
